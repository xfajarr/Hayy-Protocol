...
type UseMutateMintMockSbtcParams = {
  amount: number;
};

export function useMutateMintMockSbtc() {
  const currentAccount = useCurrentAccount();
  const { mutateAsync: signAndExecute } = useSignAndExecuteTransaction();
  const suiClient = useSuiClient();

  return useMutation({
    mutationFn: async (props: UseMutateMintMockSbtcParams) => {
      if (!currentAccount) throw new Error("No connected wallet");

      const tx = new Transaction();
      tx.moveCall({
        target: `${ORIGINAL_PACKAGE_ID}::${FAUCET_POOL_MODULE_NAME}::mint_sbtc`,
        arguments: [
          tx.object(FAUCET_POOL_ID),
          tx.pure.u64(props.amount * Math.pow(10, 8)),
        ],
      });
